...
public class ReadFile extends Observable {
	 ...
	 * @throws Exception
	 */
	public void readFileByLine(String fullPath, long start, long end)
			throws Exception {
		File fin = new File(fullPath);
		if ((end - start) < bufSize && end > 0) {
				...
				while (fcin.read(rBuffer) != -1) {
					...
					while ((endIndex = indexOf(newStrByte, fromIndex)) != -1) {
						byte[] bLine = substring(newStrByte, fromIndex,
								endIndex);
						line = new String(bLine, 0, bLine.length, encode).trim();
						lineNum++;
						// 输出一行内容，处理方式由调用方提供
